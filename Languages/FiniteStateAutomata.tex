...
\begin{tikzpicture}[>={Stealth[width=6pt,length=9pt]}, accepting/.style={double distance = 2pt, outer sep = 1pt + \pgflinewidth}, shorten >=1pt, auto]
  \draw (50.0pt, -75.0pt)node[state, initial, initial text =](0){$s_{0}$};
  \draw (300.0pt, -75.0pt)node[state](1){$s_{5}$};
  \draw (300.0pt, -300.0pt)node[state](2){$s_{10}$};
  \draw (550.0pt, -75.0pt)node[state](3){$s_{15}$};
  \draw (550.0pt, -300.0pt)node[state](4){$s_{20}$};
  \path[->] (1) edge node{10c/$\lambda$}(3);
  \path[->] (0) edge node{5c/$\lambda$}(1);
  \path[->] (4) edge[bend left=60] node{5c/coffee}(0);
  \path[->] (3) edge[bend right] node{10c/coffee}(0);
  \path[->] (2) edge node{10c/$\lambda$}(4);
  \path[->] (1) edge node{5c/$\lambda$}(2);
  \path[->] (4) edge[bend right] node[near start, above]{10c/coffee}(1);
  \path[->] (2) edge node{5c/$\lambda$}(3);
  \path[->] (0) edge node{10c/$\lambda$}(2);
  \path[->] (3) edge node{5c/$\lambda$}(4);
\end{tikzpicture}
}%
\end{center}
\end{frame}
...
A Meely Machine is a 6-tuple $\langle Q, \Sigma, \Gamma, q_0, \delta, \rho\rangle$ where
\begin{itemize}
  \item Q is a finite set of states,
  \item $\Sigma$ is a finite set of input symbols,
  \item $\Gamma$ is a finite set of output symbols,
...
\only<1>{State $s_{10}$ has two incoming transitions with the same output, so it needn't be split. 
Likewise, states $s_{15}$ and $s_{20}$ needn't be split.}
\only<2>{
State $s_{5}$ has two incoming transitions with different outputs $\lambda$ and $coffee$, it is split into 
$s_{5}/\lambda$ and $s_{5}/coffee$.}
\only<3>{The initial state $s_{0}$ has an incoming transition with output $coffee$. 
This initiation event is viewed as an incoming transition with output $\lambda$. So, it is split into 
$s_{0}/\lambda$ and $s_{0}/coffee$.
}
\end{overlayarea}
\end{frame}
